...
final class LoopDataManager {
    ...
    init(
        ...
        carbStore.absorptionTimeOverrun = 2
        ...
        self.doseStore.addPendingPumpEvent(event) {
            self.dataAccessQueue.async {
                ...
                do {
                    ...
                    NSLog("addConfirmedBolus: Update after confirmed bolus failed \(error)")
                }
                completion?()
            }
        }
    ...
    fileprivate func update() throws {
        ...
        let retrospectiveStart = lastGlucoseDate.addingTimeInterval(.minutes(-15)) // dm61: retrospection reduced to 15 minutes
    ...
    /// The change in glucose over the reflection time interval (default is 30 min), changed to 15 min in this branch
    ...
    struct retrospectiveCorrection {
        ...
        static var effectDuration: Double = 30
        ...
        init() {
            ...
            carbEffectLimit = 15.0 // reset integral RC if carbEffect over past 15 min is greater than carbEffectLimit expressed in mg/dL
        ...
        func updateRetrospectiveCorrection(discrepancy: Double,
            ...
            if (retrospectiveCorrection.previousDiscrepancy * discrepancy < 0 ||
                ...
                retrospectiveCorrection.effectDuration = 30.0
     ...
     */
    private func updateRetrospectiveGlucoseEffect(effectDuration: TimeInterval = TimeInterval(minutes: 30)) throws {
        dispatchPrecondition(condition: .onQueue(dataAccessQueue))
        ...
        let scaledDiscrepancy = overallRC * 30.0 / effectMinutes // scaled to account for extended effect duration
        ...
        // Changed to safe fixed default retrospection interval of 15*60 = 900 seconds
        let velocity = HKQuantity(unit: velocityUnit, doubleValue: scaledDiscrepancy / 900.0)
        ...
        NSLog("myLoop 15-min retrospective delta BG: %f", currentDeltaBG)
    ...
    private func updatePredictedGlucoseAndRecommendedBasalAndBolus() throws {
        ...
        let recommendation = predictedGlucose.recommendedBolus(
            to: glucoseTargetRange,
            suspendThreshold: settings.suspendThreshold?.quantity,
            bolusThreshold: settings.bolusThreshold?.quantity,
            sensitivity: insulinSensitivity,
            model: model,
    ...
    private func setAutomatedBolus(_ completion: @escaping (_ error: Error?) -> Void) {
        ...
        guard abs(recommendedBolus.date.timeIntervalSinceNow) < TimeInterval(minutes: 5) else {
            ...
            NSLog("setAutomatedBolus - recommendation too old")
            return
        }
        ...
        if let carbChange = lastCarbChange {
            guard abs(carbChange.timeIntervalSinceNow) > TimeInterval(minutes: 2) else {
                NSLog("setAutomatedBolus - last carbchange too close")
                completion(nil)
                return
            }
        }
        ...
        delegate.loopDataManager(self, didRecommendBolus: automatedBolus) { (result) in
            self.dataAccessQueue.async {
                ...
                case .success(let bolus):
                    NSLog("setAutomatedBolus - success: \(bolus)")
                    self.recommendedBolus = nil
                    completion(nil)
                case .failure(let error):
                    completion(error)
